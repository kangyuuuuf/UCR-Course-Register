...
var ForceLayout = /*#__PURE__*/_createClass(function ForceLayout(options) {
  ...
  this.getMass = function (node) {
    ...
    var _ref = node.layout || {},
        ...
        degree = _ref$degree === void 0 ? (_a = Utils.getDegree(node, _this.edges)) === null || _a === void 0 ? void 0 : _a.degree : _ref$degree,
        force = _ref.force;
  ...
  this.init = function () {
    ...
    _this.nodes.forEach(function (node) {
      ...
      var degreeInfo = Utils.getDegree(node, _this.edges);
      node.data.layout = Object.assign(Object.assign({}, node.data.layout), degreeInfo);

      var mass = _this.getMass(node.data);

      _this.nodePoints.set(node.id, new Point(vec, String(node.id), node.data, mass));
    });
    ...
    if (leafCluster) {
      ...
      var relativeNodesType = Utils.getRelativeNodesType(_this.nodes, nodeClusterBy);
      centripetalOptions = {
        single: 100,
  ...
  this.updateCoulombsLawOptimized = function () {
    ...
    var forces = forceNBody(nodes, coulombDisScale, repulsion);

    _this.nodes.forEach(function (node, i) {
      _this.nodePoints.get(node.id).updateAcc(new Vector(forces[i].vx, forces[i].vy));
    });
  };
  ...
  this.attractToCentre = function () {
    ...
    _this.nodes.forEach(function (node) {
      ...
      var centerVector = new Vector(x, y);
  ...
  this.addEdges = function (data) {
    try {
      ...
      for (var i = 0; i < len; i++) {
        ...
        var attr = e.data;
        var edge = new Edge(String(_this.nextEdgeId++), node1, node2, attr);
  ...
  this.getSameTypeLeafMap = function () {
    ...
    _this.nodes.forEach(function (node) {
      ...
      if (degree === 1) {
        sameTypeLeafMap[node.id] = Utils.getCoreNodeAndRelativeLeafNodes('leaf', node, _this.edges, nodeClusterBy);
      }
    });
